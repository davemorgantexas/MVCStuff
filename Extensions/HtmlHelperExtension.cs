...
namespace System.Web.Mvc.Html
{
    ...
    {
        public static MvcHtmlString DisplayForMany<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, IEnumerable<TValue>>> expression, string templateName = null) where TModel : class
        {
            ...
            foreach (TValue item in items)
            {
                ...
                // The field names are not as important in the display, but it might make your javascript happier.
                var itemFieldName = String.Format("{0}[{1}]", fullHtmlFieldPrefix, index++);
                string singleItemHtml = html.DisplayFor(singleItemExpression, templateName, itemFieldName).ToString();
